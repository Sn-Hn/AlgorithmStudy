...
public class 골목대장호석_기능성_20168 {
	...
	private static class Cost {
		...
		public Cost(int index, int cost, int max) {
			this.index = index;
			this.cost = cost;
			this.max = max;
		}
		
		public Cost(int index, int cost, int max, boolean[] visited) {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < M; i++) {
			...
			crossRoad[a].add(new Cost(b, c, c));
			crossRoad[b].add(new Cost(a, c, c));
		}
	...
	private static void getMinCostSection() {
		...
		q.add(new Cost(A, 0, 0));
		
		while (!q.isEmpty()) {
			Cost cost = q.poll();
			int now = cost.index;
			boolean[] visited = cost.visited;
...
//				if (visited[now] && cost.cost + next.cost > C) {
//					continue;
//				}
				q.add(new Cost(next.index, cost.cost + next.cost, Math.max(cost.max, next.cost), visited));															
			}
		}
	}
